...
def load_models():
    ...
    rec_paths = [
        BASE_DIR / 'models/action_recommendation_model.pkl',
        BASE_DIR / 'models/recommendation_engine/best_recommendation_model.pkl'
    ]
    ...
    for rec_path in rec_paths:
        try:
            if rec_path.exists():
                ...
                st.sidebar.success(f"✅ Recommendation Model Loaded")
                break
        except Exception as e:
            continue
            ...
            if not text_col:
                ...
                if process_btn:
                    with st.spinner(f"Processing {len(df)} reviews..."):
                        ...
                        # Sentiment analysis - Handle both dict and simple model
                        sentiment_model = models['sentiment']
                        
                        if isinstance(sentiment_model, dict):
                            # Advanced model with vectorizer and features
                            sentiments = []
                            for idx, row in df.iterrows():
                                # TF-IDF features
                                tfidf_feat = sentiment_model['vectorizer'].transform([row['cleaned_text']]).toarray()
                                # Advanced features
                                adv_feat = preprocessor.extract_advanced_features(df[text_col].iloc[idx])
                                # Combine features
                                X_input = np.hstack([tfidf_feat, adv_feat])
                                # Predict
                                sentiment = sentiment_model['model'].predict(X_input)[0]
                            ...
                            # Simple model
                            df['sentiment'] = sentiment_model.predict(df['cleaned_text'])
                        ...
                        if negative_mask.any() and 'recommendation' in models:
                            negative_texts = df.loc[negative_mask, 'cleaned_text'].tolist()
                            
                            # Predict using recommendation model
                            predicted_labels = models['recommendation'].predict(negative_texts)
                            
                            # Decode labels if label encoder exists
                            if 'label_encoder' in models:
                                actions = models['label_encoder'].inverse_transform(predicted_labels)
                            else:
                                actions = predicted_labels
                        ...
                        # Most Critical Reviews Section
                        st.divider()
                        st.subheader("🚨 Most Critical Reviews - Immediate Action Required")
                        
                        # Filter negative reviews
                        critical_reviews = df[df['sentiment'] == 'negative'].copy()
                        
                        if len(critical_reviews) > 0:
                            st.info(f"Found **{len(critical_reviews)}** negative reviews requiring attention")
                            
                            # Show top critical reviews (limit to 10 for display)
                            display_count = min(10, len(critical_reviews))
                            
                            for idx, row in critical_reviews.head(display_count).iterrows():
                                # Create a styled card for each critical review
                                with st.container():
                                    col1, col2, col3 = st.columns([2, 1, 1])
                                    
                                    with col1:
                                        st.markdown(f"""
                                        <div style="background: rgba(239, 68, 68, 0.1); border-left: 4px solid #ef4444; padding: 15px; border-radius: 8px; margin: 10px 0;">
                                            <p style="color: #fca5a5; margin: 0; font-size: 0.9em;">
                                                <strong>Review:</strong><br>
                                                {row[text_col][:200]}{'...' if len(row[text_col]) > 200 else ''}
                                            </p>
                                        </div>
                                        """, unsafe_allow_html=True)
                                    ...
                                    with col2:
                                        ...
                                        <div style="background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); padding: 12px; border-radius: 8px; text-align: center;">
                                            <p style="color: #93c5fd; margin: 0; font-size: 0.85em; font-weight: 500;">Category</p>
                                            <p style="color: #60a5fa; margin: 5px 0 0 0; font-size: 1.1em; font-weight: 700;">{row['category']}</p>
                                        </div>
                                        """, unsafe_allow_html=True)
                                    
                                    with col3:
                                        action = row['recommended_action'] if row['recommended_action'] else 'No action specified'
                                        st.markdown(f"""
                                        <div style="background: rgba(168, 85, 247, 0.1); border: 1px solid rgba(168, 85, 247, 0.3); padding: 12px; border-radius: 8px; text-align: center;">
                                            <p style="color: #c4b5fd; margin: 0; font-size: 0.85em; font-weight: 500;">Action</p>
                                            <p style="color: #a78bfa; margin: 5px 0 0 0; font-size: 0.95em; font-weight: 600;">{action}</p>
                                        </div>
                                        """, unsafe_allow_html=True)
                            
                            # If there are more than 10, show a summary
                            if len(critical_reviews) > display_count:
                                st.warning(f"⚠️ Showing top {display_count} critical reviews. See full data table below for all {len(critical_reviews)} negative reviews.")
                            
                            # Action Summary Table
                            st.divider()
                            st.subheader("📋 Action Summary by Category")
                            
                            # Group by category and action
                            action_summary = critical_reviews.groupby(['category', 'recommended_action']).size().reset_index(name='count')
                            ...
                            st.success("✅ No critical negative reviews found! All feedback is positive or neutral.")
